...
/*=========================================================
Get risk differenc and 95% confidence interval
=========================================================*/
...
/*----------------------------------------------------------
```````````Run proc freq with required options (riskdiff on tables statment)
----------------------------------------------------------*/
...
ods output riskdiffcol1=riskdiff01;   /*output riskdiff*/
 
proc freq data=events03;
    by aedecod label;
    tables treatment*event/riskdiff;
    weight count;
run;
...
data riskdiff02;
    set riskdiff01(rename=(risk=value));
    where row="Difference";     /*select specific stats*/
    length risk est ll ul $30;
 
    if value ne . then est=put(round(value,0.001),7.3);
    else est="NE";
    if lowercl ne . then ll=put(round(lowercl,0.001),7.3);
    else ll="NE";
    if uppercl ne . then ul=put(round(uppercl,0.001),7.3);
    else ul="NE";
    risk=strip(est)||" ("||strip(ll)||","||strip(ul)||")";
run;
 
/*----------------------------------------------------------
`````````````Merge the risk difference dataset to the dataset containing treatments as columns
----------------------------------------------------------*/
...
data trans03;
   merge trans02(in=a) riskdiff02(in=b keep=aedecod label risk);
   by aedecod label;
   if a;
   if aedecod="" and label="Overall" then ord=1;
   else ord=2;
   if missing(risk) then risk="-";
run;
...
proc report data=trans03 nowd headline headskip missing style(report)=[just=center]
   ...
   define risk/"RD (95% CI)" style(column)=[cellwidth=1.2in just=center];
