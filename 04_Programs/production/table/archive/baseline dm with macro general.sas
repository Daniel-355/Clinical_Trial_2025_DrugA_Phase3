...
=========================================================*/
%let output=C:\Users\hed2\Downloads\sdtm-adam-pilot-project-master\sdtm-adam-pilot-project-master\updated-pilot-submission-package\900172\m5\datasets\cdiscpilot01\analysis\adam\datasets\mytfl\output;
...
data adsl01;
   set adsl;
   where saffl="Y";   /*at least one treatment*/
run;
...
proc format;
   value treatment      /*"define treatment for any groups"*/
      0=1
      54=2
      81=3
98=98;
...
data adsl03;
   set adsl02;
   keep usubjid treatmentn sexn  aracen age weightbl  heightbl  bmibl ;
sexn=input(input(sex, $genderfmt.),8.);
aracen=input(input(race,$racefmt.),8.);

treatmentn=  input (put(treatment, treatment.) ,8.);      /*define treatment  1,2,3,98;*/
...
proc sql noprint;
   select count(distinct usubjid) into :n1 from adsl03 where treatment=1;
   select count(distinct usubjid) into :n2 from adsl03 where treatment=2;
   select count(distinct usubjid) into :n3 from adsl03 where treatment=3;  
   select count(distinct usubjid) into :n4 from adsl03 where treatment=98;  /*create four  macro variables based on adsl03*/
quit;
...
proc summary data=adsl03  nway;     /*age summary stats*/  /*proc summary like proc mean*/
...
title3 bcolor="#e6f9ec" "Copyright @ mycsg";
 
ods listing close;
options orientation=landscape nodate nonumber nobyline;
ods rtf file="&output.\dm_macro.rtf" style=csgpool01;
...
proc report data = final center headline headskip nowd split='~' missing style(report)=[just=center]
   ...
   define c4/"Active54" "(N=%cmpres(&n2))"  style(column)=[cellwidth=1.7in just=center]   ;
   define c5/"Active81" "(N=%cmpres(&n3))"  style(column)=[cellwidth=1.7in just=center]   ;
