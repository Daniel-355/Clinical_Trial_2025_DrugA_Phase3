...
ods listing close;
ods rtf file="&outputpath.\lb_change.rtf" style=csgpool01;
 
 
title "Actual and Change from Baseline for Laboratory Tests";
title2 "Safety Analysis Set";
title3 bcolor="#e6f9ec" "Copyright @ mycsg.in";
title4 ' ';
 
proc report data=stat08 nowd headline headskip missing style(report)=[just=center] split='~';
   columns page paramn avisitn statorder statistic
   ("(*ESC*)S={borderbottomcolor=black borderbottomwidth=1 just=c} Completers of Week 24~(N=%cmpres(&n1))" trt_11 trt_12 trt_13)
   dummy
   ("(*ESC*)S={borderbottomcolor=black borderbottomwidth=1 just=c} Total~(N=%cmpres(&n2))" trt_21 trt_22 trt_23);
